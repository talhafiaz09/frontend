...
class VoiceModal extends Component {
  ...
  onSpeechResults(e) {
    ...
    if (this.props.ingredientsList && this.state.spokenText.length > 0) {
      this.props.ingredientsList.forEach((array) => {
        let intersection = this.state.spokenText.filter((x) =>
          array.includes(x),
        );
    ...
    console.log(this.state.userpantriesname);
  }
  componentDidMount() {
